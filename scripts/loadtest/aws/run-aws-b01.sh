...
ADAPTIVE_BREAKPOINT="$(python3 -c 'import json,sys; print("1" if json.load(open(sys.argv[1])).get("profileVersion") in {"aws-eks-monolith-breakpoint-v2.0", "aws-eks-monolith-breakpoint-v2.1", "aws-eks-monolith-msa-boundary-v1.0"} else "0")' "$AWS_PROFILE_FILE")"
