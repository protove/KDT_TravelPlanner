...
lines = [
    ...
    'curl --fail --silent --show-error --max-time 3 -X POST -H "Content-Type: application/json" --data "{}" http://127.0.0.1:8080/v1/places:searchText >/dev/null',
    'curl --fail --silent --show-error --max-time 3 -X POST -H "Content-Type: application/json" --data "{}" http://127.0.0.1:8080/v1/places:searchNearby >/dev/null',
    'curl --fail --silent --show-error --max-time 3 http://127.0.0.1:8080/v1/places/loadtest-place >/dev/null',
    'curl --fail --silent --show-error --max-time 3 -X POST -H "Content-Type: application/json" --data "{}" http://127.0.0.1:8080/directions/v2:computeRoutes >/dev/null',
...
lines = [
    ...
    'MOCK_BODY="$BASE_ROOT/.mock-body.json"',
    'curl --fail --silent --show-error --max-time 3 -X POST -H "Content-Type: application/json" --data "{}" --output "$MOCK_BODY" http://127.0.0.1:8080/v1/places:searchText',
    'python3 - "$MOCK_BODY" <<\'PYPLACESSEARCH\'\nimport json\nimport sys\npayload = json.load(open(sys.argv[1]))\nif not isinstance(payload.get("places"), list) or not payload["places"]:\n    raise SystemExit("places search body contract failed")\nPYPLACESSEARCH',
    'curl --fail --silent --show-error --max-time 3 -X POST -H "Content-Type: application/json" --data "{}" --output "$MOCK_BODY" http://127.0.0.1:8080/v1/places:searchNearby',
    'python3 - "$MOCK_BODY" <<\'PYPLACESNEARBY\'\nimport json\nimport sys\npayload = json.load(open(sys.argv[1]))\nif not isinstance(payload.get("places"), list) or not payload["places"]:\n    raise SystemExit("places nearby body contract failed")\nPYPLACESNEARBY',
    'curl --fail --silent --show-error --max-time 3 http://127.0.0.1:8080/v1/places/loadtest-place >/dev/null',
    'curl --fail --silent --show-error --max-time 3 -X POST -H "Content-Type: application/json" --data "{}" --output "$MOCK_BODY" http://127.0.0.1:8080/directions/v2:computeRoutes',
    'python3 - "$MOCK_BODY" <<\'PYROUTES\'\nimport json\nimport sys\npayload = json.load(open(sys.argv[1]))\nif not isinstance(payload.get("routes"), list) or not payload["routes"]:\n    raise SystemExit("routes body contract failed")\nPYROUTES',
    'rm -f "$MOCK_BODY"',
    'K6_IMAGE_ID="$(docker image inspect --format "{{.Id}}" "$K6_IMAGE")"',
    'MOCK_IMAGE_ID="$(docker image inspect --format "{{.Id}}" "$MOCK_IMAGE")"',
