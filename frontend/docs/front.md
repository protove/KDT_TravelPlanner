...
브레이크포인트는 Tailwind 기본값을 그대로 쓴다(별도 `--breakpoint-*` 재정의 없음): `sm` 640 · `md` 768 · `lg` 1024 · `xl` 1280.

`md`(768px) 미만을 모바일 기준으로 삼는다. 대부분의 화면이 아직 데스크톱 고정 레이아웃이므로, 반응형 전환은 컴포넌트를 손댈 때마다 점진적으로 진행한다. 임의 px 값으로 미디어쿼리를 만들지 않고 `sm:`/`md:`/`lg:` prefix만 사용한다.

`MyPageLayout` 탭 내비게이션: `md` 이상에서는 좌측 세로 사이드바(고정폭 180px)를 유지한다. `md` 미만에서는 동일한 버튼 마크업을 상단 가로 스크롤 탭바로 전환한다. 공용 `Tabs` atom으로 바꾸지 않는 이유는 그 atom이 `trips` 목록/`ScheduleBoard` 날짜 탭과 공유돼 있어 스타일 변경이 그 화면들까지 번지기 때문 — `MyPageLayout` 전용 버튼 마크업을 유지해 영향 범위를 마이페이지로 한정한다. 탭이 컨테이너 폭을 넘쳐 스크롤이 가능할 때만 오른쪽에 옅은 페이드 + 화살표 힌트를 보여주고, 다 들어가는 경우(예: 탭 2개)엔 힌트를 표시하지 않는다.

## 7. AI 작업 규칙 (일관성 보장의 핵심)
