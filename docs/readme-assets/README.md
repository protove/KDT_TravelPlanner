...
- `aws-dev-architecture`: `dev`, EC2 `dev-runtime`, 종속 `dev-load-test`의 독립 Root/State
- `delivery-observability`: 변경 전달과 로컬 검증·증거 흐름
- 공통 규칙: 16:9, 밝은 slate 배경, navy 제목, blue `PERSISTENT`, green `EC2 RUNTIME`,
  amber 점선 `ON DEMAND` 배지
- 화살표 규칙: 시작점과 끝점이 카드 경계에 닿고, 흐름의 방향이 한쪽으로만 읽혀야 하며,
  빈 공간에서 시작하거나 카드를 가로지르거나 카드 내부로 들어가지 않게 한다.
- State 비교 규칙: 독립 Root/State 경계는 카드로 분리하고, 원격 State 의존성이 있을 때만
  방향과 생성 순서를 명시한 점선 연결을 사용한다.
