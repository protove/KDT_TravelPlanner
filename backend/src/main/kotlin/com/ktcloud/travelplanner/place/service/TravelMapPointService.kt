...
class TravelMapPointService(
	...
	fun getMapPoints(
		...
		if (dayNumber !in 1..travel.travelDays) throw InvalidMapDayNumberException()
		...
		timelineItems.forEach { timelineItem ->
			val googlePlaceId = timelineItem.googlePlaceId?.takeIf { it.isNotBlank() }
			...
			points += TravelMapPointResponse.from(timelineItem, location)
		}
...
class MapPointTravelNotFoundException : DomainException(ErrorCode.RESOURCE_NOT_FOUND)

class MapPointAccessDeniedException : DomainException(ErrorCode.ACCESS_DENIED)

class InvalidMapDayNumberException : DomainException(
