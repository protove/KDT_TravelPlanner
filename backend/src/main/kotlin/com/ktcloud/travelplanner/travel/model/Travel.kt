...
class Travel(
        ...
        // 여행 목적/취향 복수 선택 태그 (별도 Entity 없이 값 컬렉션으로 관리)
        @ElementCollection(fetch = FetchType.LAZY)
        @CollectionTable(name = "planner_purposes", joinColumns = [JoinColumn(name = "planner_id")])
        @Enumerated(EnumType.STRING)
        @Column(name = "purpose")
        var purposes: MutableSet<TravelPurpose> = mutableSetOf()
                protected set
